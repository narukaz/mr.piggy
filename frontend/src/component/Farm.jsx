...
const ActivePiggies = ({ piggies, searchTerm }) => {
      ...
      {filteredPigs.length > 0 ? (
        filteredPigs.map((pig) => {
          const nextCycleDate = new Date(
            pig.nextExpectedPaymentTimeRange * 1000
          ).toLocaleDateString();
          ...
          return (
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: `url(${getFrameSvg(pig.piggyHealth)})`,
                }}
              />
  ...
  useEffect(() => {
    const fetchPiggyData = async () => {
      ...
      try {
        if (activeTab === "idle") {
          ...
          const formattedActivePiggies = tokenIds.map((id, index) => {
            ...
            return {
              ...
              depositCount: Number(data.depositCount),
              startTime: Number(data.startTime),
              lastDeposit: Number(data.lastDeposit),
              pledgeAmount: data.pledgeAmount,
              cycle: Number(data.cycle),
              nextExpectedPaymentTimeRange: Number(
                data.nextExpectedPaymentTimeRange
              ),
              goalAmount: data.goalAmount,
              piggyHealth: Number(data.piggyHealth),
            };
          });
        ...
        setError("Failed to fetch your piggies. Please try again.");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchPiggyData();
  }, [activeTab, account.address, config, status]); // Added status to refetch after successful activation
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-8 mt-48">
        <div className="flex flex-col gap-8 bg-[#F7F7F7] rounded-3xl px-8 md:px-16 py-12 w-full max-w-5xl">
          ...
          {!isLoading && !error && activeTab === "active" && (
            <ActivePiggies piggies={activePiggies} searchTerm={searchTerm} />
          )}
